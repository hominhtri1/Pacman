...
Agent2::Agent2(vector<vector<int>> map)
{
	this->map = map;
...
vector<Pos> Agent2::buildPath(vector<vector<Pos>> & prev, int end_x, int end_y)
{
	vector<Pos> curPath;
...
void Agent2::findPathWithBFS()
{
	...
	while (D.size() > 0)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (map[new_x][new_y] == 1 || map[new_x][new_y] == 3)
				continue;
